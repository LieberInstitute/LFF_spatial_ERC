`AHBA_PFC_filtered.json` Cell type marker genes from PsychENCODE cell type annotation pipeline [link](https://github.com/gersteinlab/PsychENCODE_SingleCell_Integrative/blob/ef5b289f123820edfb34741d018de133ab9c16cd/scRNA-seq%20Pipeline/AHBA_PFC_filtered.json)

`Allen_Brain_Mouse_cortex_hippo`

`Grubman2019` Supplementary tables from Grubman et al., 2019 [10.1038/s41593-019-0539-4](https://doi.org/10.1038/s41593-019-0539-4)
DE data downloaded from http://adsn.ddnetbio.com "AD vs Control cells within each cell type". LFC cutoff 0.5 (minimum option) FDR cutoff 0.05.

`Mathys2016`  Supplementary tables from Mathys et al., 2019 [10.1038/s41586-019-1195-2](https://doi.org/10.1038/s41586-019-1195-2)

`OpenTargets` Alzheimer disease associated gene list Downloaded May 21, 2025
 [link](https://platform.opentargets.org/disease/MONDO_0004975/associations)

`Ramsden2015` Supplementary tables from Ramsden et al., 2015 [10.1371/journal.pcbi.1004032](https://doi.org/10.1371/journal.pcbi.1004032)

`Sadick 2022` Supplementary tables from Sadick et al. 2022 [10.1016/j.neuron.2022.03.008](https://doi.org/10.1016/j.neuron.2022.03.008)

`Lou2023` Supplementary tables from Lou et al., 2023 [10.1002/advs.202300876](https://doi.org/10.1002/advs.202300876)
