...
tech_var_mod <- model.matrix(~Sex + Age + Rin + exp_round + seq_round + sum + detected + subsets_Mito_percent, colData(sce))
dim(tech_var_mod)
head(tech_var_mod)
...
pdf(here(plot_dir, "nmf_tech_var_cor_heatmap.pdf"), height = 11)
Heatmap(nmf_tech_cor)
dev.off()

nmf_anno_tech_var <- annotate_registered_clusters(
    cor_stats_layer = nmf_tech_cor,
...
nmf_anno_tech_var |> filter(cluster == "nmf21")

#### Demographic Vars ####

demo_var_mod <- model.matrix(~BrNum, colData(sce))
head(demo_var_mod)

demo_var_mod <- as.data.frame(demo_var_mod)
demo_var_mod[,"(Intercept)"] <- NULL

colnames(demo_var_mod) <- gsub("BrNumBr", "Br", colnames(demo_var_mod))
...
pdf(here(plot_dir, "nmf_demo_var_cor_heatmap.pdf"), height = 11)
Heatmap(nmf_demo_cor)
dev.off()

nmf_anno_demo_var <- annotate_registered_clusters(
    cor_stats_layer = nmf_demo_cor,
    confidence_threshold = 0.5,
    cutoff_merge_ratio = 0.1
)
...
pdf(here(plot_dir, "nmf_cellType_var_cor_heatmap.pdf"), height = 11)
...
nmf_anno_cellType_var |> filter(layer_confidence == "good")
