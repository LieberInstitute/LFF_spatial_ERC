...
compare_contrast_stats <- function(dge_tb, stat = "t", m = "vlmf", FDR_cut = 0.05, risk = FALSE, datatype = opt$datatype){
    ...
    cor <- dge_tb_wide |>
        group_by(cluster) |>
        summarise(cor = cor.test(!!sym(paste0(stat,"_carrier_M")), !!sym(paste0(stat,"_carrier_F")))$estimate,
                  p_val = cor.test(!!sym(paste0(stat,"_carrier_M")), !!sym(paste0(stat,"_carrier_F")))$p.value) |>
        ungroup() |>
    ...
    if(risk){
        ...
        plot_fn = sprintf("Sex_%s_stat_scatter_%s_risk.png", datatype, stat)
    } else {
        stat_scatter <- stat_scatter + geom_text_repel(aes(label = ifelse(DE_class != "None", gene_name, "")), size = 1.5)
        plot_fn = sprintf("Sex_%s_stat_scatter_%s.png", datatype, stat)
    }
