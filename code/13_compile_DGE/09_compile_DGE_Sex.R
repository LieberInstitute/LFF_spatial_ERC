...
## Compile and plot all DGE data from carrier_Sex contrast
...
data_dir <- here("processed-data", "13_compile_DGE", "09_compile_DGE_Sex", opt$datatype)
if (!dir.exists(data_dir)) dir.create(data_dir, recursive = TRUE)

plot_dir <- here("plots", "13_compile_DGE", "09_compile_DGE_Sex", opt$datatype)
...
vlmf_fn <- list.files(here("processed-data", "12_voomLmFit", "05_Clusterwise_voomLmFit_Sex", paste0("vlmf_",opt$datatype)),
                      full.names = TRUE, pattern = ".rds")

names(vlmf_fn) <- map_chr(vlmf_fn, ~gsub("voomLmFit_carrier_sex_sn_broad_|voomLmFit_carrier_sex_sn_fine_|voomLmFit_carrier_sex_Visium_|.rds", "", basename(.x)))
...
vlmf_data_tb <- map_dfr(vlmf_data, ~do.call("rbind", .x[c("carrier_F", "carrier_M")]) |>
...
vlmf_model_summary_bar <- vlmf_model_summary |>
    ...
    theme_bw() +
    labs(title = sprintf("voomLmFit - %s - carrier by Sex", opt$datatype), subtitle = "FDR < 0.05") +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
          legend.position = "None")

ggsave(vlmf_model_summary_bar, filename = here(plot_dir, sprintf("Sex_%s_vlmf_model_summary_bar.png", opt$datatype)))
...
ggsave(vlmf_model_summary_bar_reg, filename = here(plot_dir, sprintf("Sex_reg_%s_vlmf_model_summary_bar.png", opt$datatype)))

## save summary
write.csv(vlmf_model_summary, file = here(data_dir, sprintf("vlmf_Sex_model_summary_%s.csv", opt$datatype)))
...
if(opt$datatype == "sn_fine"){
    ...
    map(cell_type_broad_levels, ~vlmf_data_tb |>
            ...
            custom_volcano(model_name = paste0("sn_fine-", .x, "-Sex")))
    
    map(cell_type_broad_levels, ~vlmf_data_tb |> 
            filter(grepl(.x, cluster),
                   gene_name %in% AD_risk$symbol) |>
            custom_volcano(model_name = paste0("sn_fine-", .x, "-Sex-risk")))
    
} else {
    ## plot volcanos
    custom_volcano(data = vlmf_data_tb, model_name = paste0(opt$datatype, "-Sex"))
    
    ## filter to risk genes
    custom_volcano(data = vlmf_data_tb|> filter(gene_name %in% AD_risk$symbol), model_name = paste0(opt$datatype, "-Sex-risk"))
}


#### save data ####

saveRDS(vlmf_data_tb, file = here(data_dir, sprintf("DGE_results_Sex_%s.Rds", opt$datatype)))
write.csv(vlmf_data_tb, file = here(data_dir, sprintf("DGE_results_Sex_%s.csv", opt$datatype)), row.names = FALSE)

# vlmf_data_tb <- readRDS(here(data_dir, sprintf("DGE_results_Sex_%s.Rds", opt$datatype)))
...
compare_carrier_stats <- function(dge_tb, stat = "t", m = "vlmf", FDR_cut = 0.05, risk = FALSE, datatype = opt$datatype){
    ...
    ## combine data
    dge_combined <- dge_tb  |>
        select(cluster, contrast, gene_id, gene_name, t_Sex = vlmf_t, fdr_Sex = vlmf_adj.P.Val) |>
        left_join(carrier_data, by = join_by(cluster, gene_id))
    
    # dge_combined |> group_by(cluster)|> count(is.na(t_Sex), is.na(t_carrier))
    # dge_combined |> filter(is.na(t_Sex) | is.na(t_carrier)) |> group_by(cluster)|> count(is.na(t_Sex), is.na(t_carrier))
    ...
    ## calc correlation
    cor <- dge_combined |> 
        filter(!is.na(t_Sex) & !is.na(t_carrier)) |>
        group_by(cluster, contrast) |>
        summarise(cor = cor.test(t_Sex, t_carrier)$estimate,
                  p_val = cor.test(t_Sex, t_carrier)$p.value) |>
        ungroup() |>
    ...
    # dge_combined <- dge_combined |> replace_na(list(t_carrier = -Inf, t_Sex = -Inf))
    ...
    if(risk){
        ...
        plot_fn = sprintf("Sex_v_carrier_%s_stat_scatter_%s_risk.png", datatype, stat)
    } else {
        stat_scatter <- stat_scatter + geom_text_repel(aes(label = ifelse(DE_class != "None", gene_name, "")), size = 1.5)
        plot_fn = sprintf("Sex_v_carrier_%s_stat_scatter_%s.png", datatype, stat)
    }
...
if(opt$datatype == "sn_fine"){
    ...
    # ## compare t-stats
    compare_contrast_stats(vlmf_data_tb, contrast_1 = "carrier_F", contrast_2 = "carrier_M", my_text_size = 1.5)
    compare_contrast_stats(vlmf_data_tb, contrast_1 = "carrier_F", contrast_2 = "carrier_M", my_text_size = 1.5, risk = TRUE)
    ...
    ## compare logFC
    # compare_contrast_stats(vlmf_data_tb, stat = "logFC", contrast_1 = "carrier_F", contrast_2 = "carrier_M")
}
