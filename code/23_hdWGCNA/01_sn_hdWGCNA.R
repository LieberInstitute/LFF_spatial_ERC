...
if(!file.exists(here(data_dir, 'hdWGCNA_object.rds'))){
  #### Read in Data ####
  ...
  #### load saved hdWGCNA data ####
  message(Sys.time(), " - load saved hdWGCNA data")
  seurat_obj <- readRDS(here(data_dir, 'hdWGCNA_object.rds'))
}
...
GetNetworkData(seurat_obj, wgcna_name = "ERC_sn")$TOMFiles
