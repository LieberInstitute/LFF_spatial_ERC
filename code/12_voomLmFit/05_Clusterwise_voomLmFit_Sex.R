...
## Run VoomLmFit on ERC clusters E2 vs. E4 by Sex
...
#### Set up dirs ####
data_dir <- here("processed-data", "12_voomLmFit", "05_Clusterwise_voomLmFit_Sex", sprintf("vlmf_%s", opt$datatype))
...
## Add Sex + APOE value
sce_pb$carrier_Sex <- paste0(sce_pb$APOE_carrier_syn, "_" ,sce_pb$Sex)
...
vlmf_summary <- map_dfr(clusters, function(clus){
    ...
    colnames(des) <- gsub(colnames(des),pattern="_Sex",replacement="_")
    ...
    cont <- makeContrasts(
        ...
        carrier_F = "-carrier_E2_F + carrier_E4_F",
        carrier_M = "-carrier_E2_M + carrier_E4_M",
        ## compare ancestry by carrier stats
        Sex_E2 = "carrier_E2_F-carrier_E2_M",
        Sex_E4 = "carrier_E4_F-carrier_E4_M",
        levels=des
    )
...
#                     create_shell = TRUE,
#                     name = "05_Clusterwise_voomLmFit_Sex",
#                     create_script = FALSE)
